...
class DataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        ...
        self.fourier_encoding = FourierEncoding(d_model=d_model)
    ...
    def forward(self, x, x_mark=None):
        ...
        x_freq = self.fourier_encoding(x_val)
        x_freq_norm = self.norm(x_freq)
        combined = torch.cat([x_val, x_freq_norm], dim=-1)
        # x = x_val + x_freq  # + x_global
        x = self.mix(combined)
        x = self.dropout(x)
        return x
