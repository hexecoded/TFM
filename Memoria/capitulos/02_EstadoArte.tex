...
Como alternativa más reciente de los modelos aditivos, disponemos de Prophet \cite{taylor2018prophet}. Desarrollado por Meta, tiene como objetivo ofrecer una solución robusta para la predicción de series temporales con una fuerte componente estacional y con tendencias no lineales. Su principal ventaja radica en que permite a los analistas realizar predicciones robustas sin requerir conocimientos avanzados en modelado de series temporales, proporcionando un marco flexible y fácilmente interpretable.

La descomposición realizada se basa en tres componentes, de manera similar a los modelos anteriores:
...
\begin{itemize}
	\item \( g(t) \): representa la {tendencia a largo plazo}, que puede ser lineal o logística, y puede contener cambios de tendencia en fechas específicas.
	\item \( s(t) \): modela la {estacionalidad} periódica mediante funciones basadas en series de Fourier. Se pueden capturar estacionalidades diarias, semanales, anuales, aunque las implementaciones disponibles permiten tambiéne especificar intervalos definidos por el usuario como desee.
	\item \( h(t) \): corresponde al modelado de fechas específicas que muestren un comportamiento diferente al resto, y pueden introducirse manualmente por el usuario.
