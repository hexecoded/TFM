...
\subsubsection{Problemáticas habituales}

Aunque las RNN suponen un gran avance en el aprendizaje de datos secuenciales, y permitieron una mejora considerable en los resultados, no están exentas de problemas durante el aprendizaje. Al igual que otro tipo de modelos que usan la propagación mediante gradientes, debemos tener especial cuidado con su gestión para evitar la explosión de gradientes, y en su caso, el extremo contrario, el desvanecimiento.\\

Estos efectos se ven potenciados debido al particular funcionamiento del backpropagation entre los diferentes pasos. A diferencia de una red neuronal clásica, necesitamos que los pesos tengan relación también con los pasos anteriores de la secuencia, y para lograrlo, debemos modificar el proceso, de manera que podamos emplear el procedimiento habitual con la nueva estructura oculta de pesos. En cada etapa de actualización:

\begin{enumerate}
	\item La RNN se desenrolla (unroll) en el tiempo durante un número finito de pasos (por ejemplo, 10).
...
De esta forma, constituimos el mecanismo conocido como \textit{Backpropagation Through Time } (BPTT), y es durante su proceso de entrenamiento cuando podemos encontrarnos ante la explosión 
