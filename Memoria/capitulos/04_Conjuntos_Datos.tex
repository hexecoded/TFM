\chapter{Selección y análisis de los conjuntos de datos empleados en el análisis experimental de las propuestas}
...
\subsection{Household Power Consumption (HPC)}
...
\subsection{Time-series Industrial Anomaly dataset (TINA)}
...
\begin{algorithm}[!ht]
	...
	\begin{algorithmic}[1]
		\Procedure{preprocesarDatos}{root\_path, data\_path, target, seq\_len, set\_type, features, scale}
		
		\State \texttt{df\_raw} $\gets \Call{leerDatos}{root\_path + data\_path}$ \Comment{Adaptado según formato}
		...
		\Comment{Marcamos las particiones de train y test}
		...
		\Comment{Seleccionamos entre enfoque multivariable vs única variable                 }
		
		\If{\texttt{features} = 'M' \textbf{or} 'MS'}
		\State \texttt{cols\_data} $\gets$ \texttt{df\_raw.columns[1:]}
		\State \texttt{df\_data} $\gets$ \texttt{df\_raw[cols\_data]}
		\ElsIf{\texttt{features} = 'S'}
		\State \texttt{df\_data} $\gets$ \texttt{df\_raw[[target]]}
		\EndIf
		
		\Comment{Se aplica escalado en caso de que se especifique}
		
		\If{scale}
		\State train\_data $\gets df\_data[border1s[0]:border2s[0], :]$
		\State $\mu \gets$ \text{media columnas de train}
		\State $\sigma \gets$ \text{desviación estándar columnas de train}
		...
		\Comment{Almacenamos toda la información procesada}
